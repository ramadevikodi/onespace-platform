 ...
 * the copyright owner.
 *
 * File name: ApplicationIntegratorAppListing.java
 */
...
@Component
public class ApplicationIntegratorAppListing implements RoleBasedAppListing {
    ...
    public Page<ApplicationEntity> listApplications(IntrospectionResponse introspectionResponse,
        ...
        Page<ApplicationEntity> applicationEntities = listApplicationsOfBU(introspectionResponse,
                String.valueOf(RoleEnum.APPLICATIONINTEGRATORROLE),
                specification,
                statusId,
                pageable);
        // filter the applications based on the status
        applicationEntities.stream().filter(application -> {
            if (application.getStatus().getName().equals(Constants.DRAFT)) {
                return application.getRegisteredBy().equals(userName);
            }
            return true;
        });
